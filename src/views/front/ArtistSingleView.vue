<template>
  <main class="container space-y-[20px] md:space-y-[58px] lg:space-y-[117px] pt-16 pb-[128px] lg:pb-[192px]">
    ...
    <section class="artist-intro mb-10 md:mb-[6.5rem] lg:mb-40">
      <div class="flex justify-between mb-5">
        <div class="flex">
          <p  
            class="mr-1"
            v-for="keyword in singleArtist?.keywords" :key="keyword"
          >
      ...
      <div class="w-[85%] lg:w-[63%] mx-auto xl:relative">
        <img 
          ...
          :src="singleArtist?.cover_urls?.horizontal" :alt="singleArtist?.name" 
        />
        <div class="flex justify-center xl:absolute bottom-[-13%] right-[-12%]">
          <div class="relative">
            <div class="bg-artistName font-lato absolute top-[-1rem] left-[1rem] text-stroke-light z-[-1]"
            >
              {{ singleArtist?.name }}
            </div>
            <div class="bg-artistName font-lato">
              {{ singleArtist?.name }}
            </div>
          </div>
        </div>
    ...
    <div class="md:flex md:justify-between md:items-center lg:w-[70%] lg:mx-auto">
      ...
      <section class="mb-[150px] md:mb-0 md:w-[45%]">
        ...
        <div class="gap-3 flex flex-row md:flex-col flex-wrap items-center md:items-end">
          <div 
            v-for="song in singleArtist?.popular_songs" :key="song" 
      ...
      <section class="artist-honors-relative md:w-[50%]">
        ...
        <div class="bg-shadow-trans-text artist-honors w-[90%] md:w-[100%] ml-auto text-center break-keep py-[50px] px-[10px] md:py-[60px] rounded-[31px] z-[-1]">
          <div 
            v-for="honor in singleArtist?.honors" 
            :key="honor"
            class="mb-[22px]"
          >
            <span class="block mb-[5px]">{{ honor }}</span> 
            <hr class="w-[65%] mx-auto">
            <!-- <span class="w-[35%] h-1 border-indigo-500"></span> -->
            <!-- <span><br />-<br /></span> -->
          </div>
        </div>
      </section>
    ...
    <section>
      ...
      <template v-for="upcoming in singleArtist?.upcoming_concerts">
        <div
      ...
      <div v-if="singleArtist?.upcoming_concerts.length === 0" class="border border-black-60 rounded-[25px] py-[10px] px-[9px] flex justify-center items-center lg:w-[70%] lg:mx-auto text-[14px] sm:text-[17px] md:text-[21px]">
        抱歉! 目前沒有資料
      </div>
    </section>
    <!-- 即將舉辦(區塊四) end -->
    ...
    <section>
      ...
      <template v-for="historical in singleArtist?.historical_concerts">
        <div
      ...
      <div v-if="singleArtist?.historical_concerts.length === 0" class="border border-black-60 rounded-[25px] px-[9px] py-[10px] flex justify-center items-center lg:w-[70%] lg:mx-auto text-[14px] sm:text-[17px] md:text-[21px]">
        抱歉! 目前沒有資料
      </div>
    </section>
    <!-- 已結束(區塊五) end -->
