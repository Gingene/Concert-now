<template>
  ...
  <!-- Collection -->
  <section class="container lg:w-[1000px] xl:w-[1200px] grid lg:grid-cols-2 gap-2 lg:gap-6 pb-20 sm:pb-[14rem] overflow-hidden">
    <!-- Artist -->
    <Card class="p-3 sm:p-7 mb-5 lg:mb-0 relative bg-shadow-trans-text rounded-[40px] border-black-60">
      ...
      <CardHeader>
        <CardTitle class="tracking-wide text-lg md:text-xl lg:text-2xl flex justify-between items-center">
          表演者追蹤清單
          <router-link :to="`/artists`" class="hover:translate-x-1 hover:-translate-y-1">
      ...
      <CardContent>
        <ScrollArea class="w-full h-[144px] lg:h-[400px]">
          ...
          <router-link v-else v-for="artist in userDynamic?.followed_artists" :key="artist.id" :to="`/artists/${artist.id}`" class="inline-block sm:py-1">
            <Badge class="badge touch-pinch-zoom sm:text-base">
              {{ artist.name }}
            </Badge>
          </router-link>
        </ScrollArea>
    ...
    <Card class="p-3 sm:p-7 relative bg-shadow-trans-text rounded-[40px] border-black-60">
      ...
      <CardHeader>
        <CardTitle class="tracking-wide text-lg md:text-xl lg:text-2xl flex justify-between items-center">
          演唱會收藏清單
          <router-link :to="`/concerts`" class="hover:translate-x-1 hover:-translate-y-1">
            <ArrowUpRightFromCircle size="30" />
          </router-link>
        </CardTitle>
      ...
      <CardContent>
        <ScrollArea class="w-full h-[280px] sm:h-[350px] lg:h-[400px]">
          ...
          <router-link v-else :to="`/concerts/${concert.id}`" v-for="concert in userDynamic?.saved_concerts" :key="concert.id + 123" class="rounded-[30px] hover:cursor-pointer">
            <div class="flex flex-row gap-6 items-center hover:-translate-y-2">
              <div class="text-base md:text-lg basis-5/6 font-lato tracking-wide">
                {{ concert.title }}
                <p class="text-[8px] sm:text-[12px] lg:text-[16px] opacity-40 mt-1">{{ concert.holding_time.split(' ')[0] }} （{{ chineseDays[new Date(concert.holding_time).getDay()] }}）</p>
              </div>
