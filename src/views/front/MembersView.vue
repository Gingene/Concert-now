<template>
  ...
  <main 
    ...
    <Card class="p-3 sm:p-7 mb-5 lg:mb-0 relative bg-shadow-trans-text rounded-[40px] border-black-60">
      <span 
        class="absolute -bottom-5 right-[10%] xs:right-[0px] text-[70px] sm:text-[90px] text-center font-black font-lato text-stroke text-black-85 -rotate-12"
        > 
        ARTISTS 
      </span>
      <CardHeader>
        <CardTitle class="tracking-wide text-lg sm:text-[22px] md:text-xl lg:text-2xl flex justify-between items-center">
          ...
          <router-link 
            :to="`/artists`" 
            class="hover:translate-x-1 hover:-translate-y-1 transition ease-in-out delay-100 duration-200"
            >
            <ArrowUpRightFromCircle size="30" />
          </router-link>
        </CardTitle>
        <CardDescription class="pt-1 text-sm md:text-base"> 
      ...
      <CardContent>
        <ScrollArea class="w-full h-[144px] lg:h-[400px]">
          <div 
            v-if="userDynamic?.followed_artists?.length === 0" 
            class="text-base text-tiffany font-semibold tracking-wider"
            >
            目前沒有追蹤的藝人喔！
          </div>
          <router-link
    ...
    <Card class="p-3 sm:p-7 relative bg-shadow-trans-text rounded-[40px] border-black-60">
      <span 
        class="absolute -bottom-10 sm:-bottom-20 right-0 text-[70px] sm:text-[90px] font-black font-lato text-stroke text-black-85 -rotate-12"
        > 
        CONCERTS 
      </span>
      <CardHeader>
      ...
      <CardContent>
        <ScrollArea class="w-full h-[280px] sm:h-[350px] lg:h-[400px]">
          <div 
            v-if="userDynamic?.saved_concerts?.length === 0" 
            class="text-base text-pink font-semibold tracking-wider"
            >
            目前沒有收藏任何演唱會喔！
          </div>
          <router-link 
