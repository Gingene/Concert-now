      ...
      <div class="lg:flex lg:justify-between lg:gap-8">
        <div class="space-y-6 lg:flex lg:flex-col lg:justify-between lg:w-[100%]">
          <div class="flex space-x-4">
            ...
            <h1 class="w-[80%] flex-auto text-xl md:text-3xl font-bold">{{ singleConcert.title }}</h1>
    ...
    <section>
      ...
      <div class="tracking-normal text-base bg-shadow-trans-text rounded-[40px] px-8 py-12 flex flex-col items-center mx-auto w-auto max-w-[350px] sm:max-w-none sm:w-[400px] lg:w-[450px] gap-2">
        ...
        <div class="flex justify-between items-center w-full">
          <!-- <div class="text-black-40">{{ singleConcert.artist.name }}</div> -->
        ...
        <ScrollArea class="h-[19rem] w-full pt-4">
          <div v-for="(song, index) in singleConcert.songs" :key="song + index">
            <div class="text-base flex justify-between items-center bg-trans">
              <div>{{ index + 1 }}</div>
              <!-- (待完成)點擊更換YT嵌入 -->
              <button class="ml-4 mr-auto py-3 max-w-[110px] sm:max-w-[160px] lg:max-w-[208px] overflow-x-hidden text-nowrap">{{ song.name }}</button>
